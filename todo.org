...
** TODO compile game lisps only if needed
<dto> hi. is there a portable way to load a lisp file only if the source is
      newer than the fasl?   [06:15]
<Xof> check the file-write-date of both source and fasl using the fancy >
      operator and the IF special form?
<Xach> dto: file-write-date might help there.
