...
** FEATURE port lighting
** FEATURE cell environment sensors?
** BUG cannot load resource "space" when no :tile ? 
** BUG bad clon error message (forward not found)
** BUG annoying music/ change to ambient/softerbeats 	  :blast:
** BUG [#A] Fix rlx:find-all-modules and standard.lisp
** BUG [#B] Don't hit own bullets 			:invader:
** BUG [#B] No damage messages when your bullets hit them :invader:
** FEATURE [#A] smarter enemies that talk	  :blast:invader:
** BUG [#A] overlapping pickups don't erase	  :invader:win32:
** FEATURE [#A] Pak text file type			 :engine:
